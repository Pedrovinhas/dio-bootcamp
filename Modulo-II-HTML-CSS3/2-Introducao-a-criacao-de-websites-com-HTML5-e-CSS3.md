...
## Prática
- Dando continuidade ao nosso site iremos montar sua estrutura. 
- Adicionaremos um cabeçalho com nosso nome, uma lista de posts (como um blog) e um rodapé para nossos contatos.

### Guia
1.Vamos abrir nosso arquivo index.html e começar pelo cabeçalho: criamos um ``<header>`` logo abaixo do ``<body>`` e colocamos o título da nossa página dentro de um `` <h1>`` .
2. Depois criaremos a lista de postagens: abrimos um elemento section e dentro dele adicionamos outro `` <header>``  contendo um `` <h2>`` . Notem que eu posso ter mais de um `` <header>``  na página.
3. Para criar nossa postagem adicionamos um ``<article>`` com um ``<header>`` e um `` <h3>`` .
4. O último passo desta etapa é criar um rodapé para nossas informações de contato: crie um elemento footer antes de fechar o `` </body>`` .
